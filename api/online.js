...
const ACTIVE_WINDOW_MS = 90 * 1000;         // "online" = ativo nos últimos 90s
...
module.exports = async function onlineHandler(req, res) {
  ...
  // CORS: como site e função ficam em domínios diferentes, devolvemos a
  ...
  // Econômico: quase sempre só 2 comandos (ZADD grava a sessão, ZCOUNT conta).
  // A limpeza e o EXPIRE rodam em ~10% das vezes, o bastante para o banco
  // não crescer. O ZCOUNT já ignora sessões antigas pela pontuação de tempo.
  const fazerLimpeza = Math.random() < 0.1;
  const commands = fazerLimpeza
    ? [
  ...
  try {
    ...
    const online = Number(results?.[fazerLimpeza ? 2 : 1]?.result);
